...
pub struct FrameTimeKeeper {
    ...
    next_frame_data_blocking_senders:
        RwLock<HashMap<String, crossbeam::channel::Sender<FrameData>>>,
    frame_data_non_blocking_senders:
...
impl FrameTimeKeeper {
    ...
    pub fn send_frame_data_to_blocking(
        &self,
        receiver_id: String,
        buffer: crossbeam::channel::Sender<FrameData>,
    ) {
        self.frame_data_blocking_senders
            .write()
            .insert(receiver_id, buffer);
    }

    pub fn send_frame_data_to_non_blocking(
        &self,
        receiver_id: String,
        buffer: tokio::sync::broadcast::Sender<FrameData>,
    ) {
        self.frame_data_non_blocking_senders
            .write()
            .insert(receiver_id, buffer);
    }

    pub fn send_next_frame_data_to_blocking(
        &self,
        receiver_id: String,
        buffer: crossbeam::channel::Sender<FrameData>,
    ) {
        self.next_frame_data_blocking_senders
            .write()
            .insert(receiver_id, buffer);
    }

    pub fn send_next_frame_data_to_non_blocking(
        &self,
        receiver_id: String,
        buffer: tokio::sync::broadcast::Sender<FrameData>,
    ) {
        self.next_frame_data_non_blocking_senders
            .write()
            .insert(receiver_id, buffer);
    }
}
...
impl WantsDeviceState for FrameTimeKeeper {
    fn on_device_added(
        &mut self,
        state: &crate::state::EmblinkenatorState,
